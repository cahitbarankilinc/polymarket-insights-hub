...
export default function PaperTradeTab({ preselectedId, prefill }: { preselectedId?: string | null; prefill?: PaperTradePrefill | null }) {
  ...
  const analysisSlippageCents = useMemo(() => {
    if (!analysisAddressId) return parseFloat(defaultConfig.slippageCents) || 0;
    return parseFloat(walletConfigs[analysisAddressId]?.slippageCents ?? defaultConfig.slippageCents) || 0;
  ...
  const analysisStats = useMemo(() => {
    ...
    const successfulTrades = myActivities.reduce((count, activity) => {
      ...
      return Math.abs(quote.diffPrice) <= (analysisSlippageCents / 100) ? count + 1 : count;
    }, 0);
              ...
              <div>
                ...
                {!collapsed && (
                  <div className="space-y-2">
                    ...
                    <label className="block">
                      <span className="mb-1 block text-[11px] text-muted-foreground">Slippage (Cent)</span>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
