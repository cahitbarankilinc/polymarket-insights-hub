...
import { debugProfileTrades, getProfileTrades, getWalletEvents, listTrackedWallets, stopWalletTracking, type ClosedTrade, type WalletTrackerEvent, type WalletTrackerInfo } from '@/lib/polymarketTrackerApi';
...
export default function TrackingTab({ onPaperTrade }: { onPaperTrade: (id: string, prefill?: PaperTradePrefill) => void }) {
  ...
  const handleDebugScrape = async (profileUrl?: string) => {
    if (!profileUrl) {
    ...
    try {
      const payload = await debugProfileTrades(profileUrl);
      toast.success(`Debug alındı • ${payload.imagePaths.length} screenshot`);
      console.log('Profile scrape debug output:', payload);
    } catch (error) {
      const message = error instanceof Error ? error.message : 'Debug scrape başarısız';
      toast.error(message);
    }
  };
                ...
                <div key={addr.id} className="glass-card p-4">
                  <div className="flex items-center justify-between gap-3 mb-3">
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        type="button"
                        onClick={() => void handleDebugScrape(addr.profileUrl)}
