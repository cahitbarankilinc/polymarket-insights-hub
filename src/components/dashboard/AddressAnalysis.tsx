  ...
  const closedTradeAnalytics = useMemo(() => {
    ...
    return {
      ...
      bins: bins.map((bin) => {
        ...
        const wonLabel = absWon > 0 ? `${bin.wonPnl.toFixed(1)} (${wonPct.toFixed(0)}%)` : '';
        const lostLabel = absLost > 0 ? `${bin.lostPnl.toFixed(1)} (${lostPct.toFixed(0)}%)` : '';
        return { ...bin, wonPct, lostPct, wonLabel, lostLabel };
      }),
    };
  }, [closedTrades]);
