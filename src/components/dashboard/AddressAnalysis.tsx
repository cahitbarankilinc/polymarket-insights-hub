  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (!active) return;
        setClosedTrades(Array.isArray(payload.trades) ? payload.trades : []);
      } catch {
        if (!active) return;
        setClosedTrades([]);
      } finally {
          ...
          <BarChart data={closedTradeAnalytics.bins} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}>
            ...
            <Tooltip
              formatter={(value: number, name: string, item) => {
                const payload = item.payload as { wonPct: number; lostPct: number };
                const pct = name === 'Won PnL' ? payload.wonPct : payload.lostPct;
                return [`${formatUsd(Number(value))} (${pct.toFixed(2)}%)`, name];
              }}
              contentStyle={{
            ...
            <Bar dataKey="wonPnl" stackId="pnl" name="Won PnL">
              ...
              <LabelList
                ...
                fill="white"
                fontSize={10}
                formatter={(value: number, _: unknown, item: { payload?: { wonPct?: number } }) => {
                  if (!value) return '';
                  return `${Number(value).toFixed(1)} (${Number(item?.payload?.wonPct ?? 0).toFixed(0)}%)`;
                }}
              />
            </Bar>
            <Bar dataKey="lostPnl" stackId="pnl" name="Lost PnL">
              ...
              <LabelList
                ...
                fill="white"
                fontSize={10}
                formatter={(value: number, _: unknown, item: { payload?: { lostPct?: number } }) => {
                  if (!value) return '';
                  return `${Number(value).toFixed(1)} (${Number(item?.payload?.lostPct ?? 0).toFixed(0)}%)`;
                }}
              />
            </Bar>
          </BarChart>
