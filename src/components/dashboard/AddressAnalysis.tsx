...
const renderStackedLabel = (props: {
  x?: number;
  y?: number;
  width?: number;
  height?: number;
  ...
  if (segmentHeight < 18) return null;
  ...
  const closedTradeAnalytics = useMemo(() => {
    ...
    return {
      ...
      bins: bins.map((bin) => {
        ...
        let showWonLabel = absWon > 0;
        let showLostLabel = absLost > 0;

        if (showWonLabel && showLostLabel) {
          const dualLabelPctThreshold = 18;
          if (wonPct < dualLabelPctThreshold || lostPct < dualLabelPctThreshold) {
            if (wonPct >= lostPct) showLostLabel = false;
            else showWonLabel = false;
          }
        }

        const wonLabel = showWonLabel ? `${wonPct.toFixed(0)}%` : '';
        const lostLabel = showLostLabel ? `${lostPct.toFixed(0)}%` : '';
