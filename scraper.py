...
def ensure_login(p, profile_dir, target_url):
    ...
    try:
        ...
        if is_logged_in(page):
            print("Oturum zaten açık. Login penceresi 5 saniye sonra kapanacak.")
            page.wait_for_timeout(5000)
            return

        print("Oturum kapalı görünüyor. Lütfen açılan tek Chrome penceresinde wallet ile giriş yapın.")
...
def parse_polymarket_profile(url, profile_dir, show_browser=False, debug_dir=None):
    ...
    with sync_playwright() as p:
        ...
        if not has_valid_session(p, profile_dir=profile_dir, target_url=url):
